...
Flags:
  ...
  -D, --download-only
    During a restore this will only download the target files from AWS
  -f, --force
  ...
  -r,  restore
    Restore a backup, requires a --awsbucket path and optional --backupdir
  -s, --split-size
...
Examples:
  ...
  upload into the AWS Bucket, and clear old incremental and snapshot files
...
function validate() {
  ...
  if [ -z ${AWS_BUCKET} ]; then
      ...
      if ! ${AWS_UTIL} ls ${AWS_BUCKET} &> /dev/null; then
        logerror "Cannot access S3 Cloud Storage bucket ${AWS_BUCKET} make sure" \
        " it exists"
        exit 1
      fi
  fi
  if [ ${ACTION} != "inventory" ]; then
    ...
    if [ ${ACTION} = "restore" ]; then
      AWS_LS=$(${AWS_UTIL} ls ${AWS_BUCKET} | head -n1 | awk '{print $4}')
      ...
      if  grep -q 'incr' <<< "${AWS_LS}"; then
        ...
        if grep -q 'snpsht' <<< "${AWS_LS}"; then
          ...
          " location. Make sure the --awsbucket e is the full path to a specific backup"
        fi
      fi
      if grep -q "tgz" <<< "${AWS_LS}"; then
        loginfo "Detected compressed .tgz file for restore"
        COMPRESSION=true
        TAR_EXT="tgz"
        TAR_CFLAG="-z"
      fi
      if  grep -q "tbz" <<< "${AWS_LS}"; then
        loginfo "Detected compressed .tbz file for restore"
        COMPRESSION=true
        TAR_EXT="tbz"
        TAR_CFLAG="-j"
      fi
      if  grep -q "tar" <<< "${AWS_LS}"; then
      ...
      if [[ "${RESTORE_FILE}" != *.${TAR_EXT} ]] ; then
          ...
          if [[ "${RESTORE_FILE}" ==  ${TAR_EXT}-* ]]; then
            ...
            logerror "Restore is not a tar file  ${AWS_BUCKET}"
          fi
      fi
      if [[ ! ${AWS_BUCKET} =~ ^.*\.${TAR_EXT}$ ]]; then
        if ${SPLIT_FILE}; then
          #remove the trailing digits and replace the suffix
          RESTORE_FILE="${RESTORE_FILE%${SUFFIX}*}${SUFFIX}*"
          AWS_BUCKET="${AWS_BUCKET%/}/${RESTORE_FILE}"
        else
          AWS_BUCKET="${AWS_BUCKET%/}/${RESTORE_FILE}"
...
function verbose_vars() {
  ...
  logverbose "AWS_TMPDIR: ${AWS_TMPDIR}"
  logverbose "AWS UTIL: ${AWS_UTIL}"
...
# List available backups in AWS
function inventory() {
  loginfo "Available Snapshots:"
  ${AWS_UTIL} ls "${AWS_BUCKET}/backups/${HOSTNAME}/snpsht" --recursive
  #loginfo "${AWS_UTIL} ls ${AWS_BUCKET}/backups/${HOSTNAME}/snpsht --recursive"
  if [ -z $incremental_backups ] || [ $incremental_backups = false ]; then
    loginfo "Incremental Backups are not enabled for Cassandra"
  fi
  loginfo "Available Incremental Backups:"
  ${AWS_UTIL} ls "${AWS_BUCKET}/backups/${HOSTNAME}/incr" --recursive
}

# This is the main backup function that orchestrates all the options
# to create the backup set and then push it to AWS
function backup() {
...
# Copy the backup files up to the AWS bucket
function copy_to_aws() {
  loginfo "Copying files to ${AWS_BACKUP_PATH}"
  if ${DRY_RUN}; then
    if ${SPLIT_FILE}; then
      loginfo "DRY RUN: ${AWS_UTIL} cp ${COMPRESS_DIR}/${SPLIT_FILE_SUFFIX}* ${AWS_BACKUP_PATH}"
    else
      loginfo "DRY RUN: ${AWS_UTIL} cp ${COMPRESS_DIR}/${ARCHIVE_FILE} ${AWS_BACKUP_PATH}"
    fi
  else
    if ${SPLIT_FILE}; then
      ${AWS_UTIL} cp "${COMPRESS_DIR}/${SPLIT_FILE_SUFFIX}*" "${AWS_BACKUP_PATH}"
    else
      ${AWS_UTIL} cp "${COMPRESS_DIR}/${ARCHIVE_FILE}" "${AWS_BACKUP_PATH}"
    fi
  fi
}
...
function restore_get_files() {
  ...
  if ${SPLIT_FILE}; then
    restore_split_from_aws
  else
    restore_compressed_from_aws
  fi

}

# Download uncompressed backup files from AWS
function restore_split_from_aws() {
  loginfo "Downloading restore files from AWS"
  if ${DRY_RUN}; then
    loginfo "DRY RUN: ${AWS_UTIL} cp ${AWS_BUCKET} ${COMPRESS_DIR}"
  else
    ${AWS_UTIL} cp "${AWS_BUCKET}" "${COMPRESS_DIR}"
  fi
  restore_split
}

# Retrieve the compressed backup file
function restore_compressed_from_aws() {
    if ${DRY_RUN}; then
      loginfo "DRY RUN: ${AWS_UTIL} cp ${AWS_BUCKET} ${COMPRESS_DIR}"
    else
...
AWS_UTIL=$(which aws)" s3" #which gsutil script
